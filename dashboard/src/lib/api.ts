...
export type Follow = { guildId: string; guildName: string; guildIconURL: string | null; channelIds: string[]; keywordPings: Record<string, string[]>; pingOwnServer: boolean; level: DockLevel; banned: boolean };
...
export const saveHomePings = (id: string, dockId: string, follow: Pick<Follow, 'keywordPings'>) => api<{ follow: Follow }>(`/api/guilds/${id}/docks/${dockId}/home-pings`, { method: 'PUT', body: JSON.stringify(follow) });
